...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#TB_GUBN").val('${PARAM.GUBN}');
		
	    //var plantList = stringToArray("${CODELIST_SYSPLT}", '${PARAM.SB_COMP_CD}');
	    var plantList = stringToArray("${CODELIST_SYSPLT}", '1100');
	    //var plantList = stringToArray("${CODELIST_SYSPLT}");
	...
	function setInitGrid() {
	    ...
	    addField(cm,    'PLANT_CODE',        {text: '플랜트'},    150,     'text', {textAlignment: "center"} ,  {editable:false});
	    addField(cm,    'PLANT_NM',          {text: '플랜트명'},    150,     'text', {textAlignment: "center"} ,  {editable:false});
	    addField(cm,    'MAT_NUMBER',        {text: '품목코드'},    150,     'text', {textAlignment: "center"} ,  {editable:false});
	    addField(cm,    'MAT_DESC',          {text: '품목명'},     120,     'text', {textAlignment: "near"}   ,  {editable:false});
	...
	function fnSearch() {
		// 조회 요청
		searchCall(gridView, '/com/bdg/selectMatList');
	}
	...
	function loadNext() {
	    ...
	    ajaxJsonCall('<c:url value="/com/bdg/selectMatList.do"/>',  params, function(data){
	...
	function setOpenerData(rowData) {
		parentCallback('fnCallbackMatSearchPop',rowData);
	}
