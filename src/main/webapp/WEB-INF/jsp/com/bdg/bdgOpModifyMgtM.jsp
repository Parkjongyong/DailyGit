<%--
	File Name : bdgOpModifyMgtM.jsp
	Description: 예산 > 예산관리 > 경영예산관리_수정기본예산
...
function fnSearch() {
	...
	// 조회 요청
	searchCall(gridView, '/com/bdg/selectOpModifyMgtList');
}
...
function fnSave() {
	...
	if(fnSaveCheck(gridView, requiredVal, false) == true){
		if(confirm("저장 하시겠습니까?\n※금액은 원단위 입니다.\n반드시 확인 후 작업하세요.")){
			...
			saveCall(gridView, '/com/bdg/saveOpModifyMgt', 'fnSave', params);
		}
	}
}
...
function fnDelete() {
	...
	if(fnDeleteCheck(gridView) == true){
		deleteCall(gridView, '/com/bdg/deleteOpModifyMgt', 'fnDelete', params);
	}
...
function fnCancel() {
    ...
    if(confirm("확정취소 하시겠습니까?")){
    	saveCall(gridView, '/com/bdg/cancelOpModifyMgt', 'fnCancel', params);
    }
...
function fnSend() {
    ...
    if(confirm("전송 하시겠습니까?")){
    	saveCall(gridView, '/com/bdg/sendOpModifyMgt', 'fnSend', params);
    }
...
function fnSendCancel() {
    ...
    if(confirm("전송취소 하시겠습니까?")){
    	saveCall(gridView, '/com/bdg/sendCancelOpModifyMgt', 'fnSendCancel', params);
    }
 ...
 */
 function fnExcel() {
		...
		var target    = "bdgOpModifyMgtExcel";
		var width     = "1800";
		var height    = "800";
	    var scrollbar = "yes";
	    var resizable = "yes";
		
	 	fnPostPopup('/com/bdg/bdgOpModifyMgtExcel', params, target, width, height, scrollbar, resizable);
