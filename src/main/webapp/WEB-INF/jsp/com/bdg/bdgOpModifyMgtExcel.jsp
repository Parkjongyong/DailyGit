<%--
	File Name : bdgOpModifyMgtExcel.jsp
	Description:예산 > 예산관리 > 경영수정기본예산신청(엑셀)
	Modification Information
	-----------------------------------------------
	수정일               수정자            수정내용
	---------- -------- ---------------------------
	2020.12.02  박종용            최초 생성
	-----------------------------------------------
	author: 박종용
	since: 2020.12.02
--%>
...
function setInitGridView() {
    ...
    addField(cm,    'ERROR_YN',           {text:'에러여부'},       80,            'text',              {textAlignment:"center"}, {editable : false});
...
function fnSearch() {
	// 조회 요청
	searchCall(gridView, '/com/bdg/selectOpModifyMgtExcel');
}
...
function fnSave() {
	...
	if(fnSaveCheck(gridView, requiredVal, false) == true){
		if(confirm("저장 하시겠습니까?\n※금액은 원단위 입니다.\n반드시 확인 후 작업하세요.")){
			...
			saveCall(gridView, '/com/bdg/saveOpModifyMgtExcel', 'fnSave', params);
		}
	}
}
 ...
 function fnUpload() {
	...
	if(fnSaveCheck(gridView, requiredVal) == true){
	     if(confirm("업로드 하시겠습니까?")){
	    	 saveCall(gridView, '/com/bdg/saveOpModifyMgt', 'fnUpload', params);
	     }
	}
}
