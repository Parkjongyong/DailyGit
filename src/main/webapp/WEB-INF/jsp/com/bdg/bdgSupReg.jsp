<%--
	File Name : bdgSupReg.jsp
	Description:예산 > 부가정보 > 공급업체등록
	Modification Information
	-----------------------------------------------
	수정일               수정자            수정내용
	---------- -------- ---------------------------
	2020.09.11  박종용            최초 생성
	-----------------------------------------------
	author: 박종용
	since: 2020.09.11
--%>
...
function fnCallbackDeptSearchPop(rows) {
 ...
 function fnApply() {
	...
	for(var i = 0; i < params.ITEM_LIST.CHECK_LIST.length; i++){
		if(params.ITEM_LIST.CHECK_LIST[i].STATUS != '1'){
			alert("저장 건만 신청 가능합니다.");
			return false;
		}
	}
	...
	if(fnSaveCheck(gridView, requiredVal) == true){
	     if(confirm("신청 하시겠습니까?")){
	    	 saveCall(gridView, '/com/bdg/applySupReg', 'fnApply', params);
	     }
	}
 }
  ...
  * 신청 성공
  */
 function fnApplySuccess(result) {
 	alert("신청 하였습니다.");
 	fnSearch();
 }
  ...
  * 신청취소
  */
 function fnApplyCancel() {
 	var params = {};
 	$.extend(params, fnGetParams());
	$.extend(params, {"ITEM_LIST" : fnGetGridCheckData(gridView)});

	if(isEmpty(params.ITEM_LIST.CHECK_LIST)){
		alert("신청취소할 대상이 없습니다.");
		return false;
	}
	...
	if(fnSaveCheck(gridView, requiredVal) == true){
	     if(confirm("신청취소 하시겠습니까?")){
	    	 saveCall(gridView, '/com/bdg/applyCancelSupReg', 'fnApplyCancel', params);
	     }
	}
 }
  ...
  * 신청 성공
  */
 function fnApplyCancelSuccess(result) {
 	alert("신청취소 하였습니다.");
 	fnSearch();
 }
  ...
  * 신청 실패
  */
 function fnApplyCancelFail(result) {
	 alert(result.errMsg);
 }	
        ...
        <table class="tbl-search">
            ...
            <tbody>
                <tr>
                    <th><span>회사</span></th>
                    <td>
	                    <select id="SB_COMP_CD" name="SB_COMP_CD" disabled></select>
                    </td>
...
<div class="sub-tit">
    <div class="btnArea t_right">
        ...
        <button type="button" class="btn" id="btnApply">신청</button>
        <button type="button" class="btn" id="btnApplyCancel">신청취소</button>
...
<!--                         <input type="text"   id="TB_POBUSI_NO" name="사업자번호" disabled> -->
                ...
                <tr>
					...
					<td colspan="3">
						...
						<label for="chk02" class="chk"><input id="chk03" type="checkbox">하나은행구매론</label>
						<label for="chk02" class="chk"><input id="chk04" type="checkbox">하나은행동반성장론</label>
						<label for="chk02" class="chk"><input id="chk05" type="checkbox">수취계좌이체</label>
						<label for="chk02" class="chk"><input id="chk06" type="checkbox">전자어음</label>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                    	<select id="SB_VENDOR_ACCOUNT" name="SB_VENDOR_ACCOUNT"></select>
                    </td>
                    <th><span>첨부</span></th>
                    <td colspan="2">
