<%--
	File Name : wrhReceivedAppr.jsp
	Description: 입고예정 > 입고승인 > 입고승인
...
function fnSearch() {
	// 조회 요청
	searchCall(gridView, '/com/wrh/selectReceivedAppr');
}
...
function loadNext() {
    ...
    ajaxJsonCall('<c:url value="/com/wrh/selectReceivedAppr.do"/>',  params, function(data){
...
var fnAppr = function(){
	...
	for(var i = 0; i < params.ITEM_LIST.UPDATED.length; i++){
		if(params.ITEM_LIST.UPDATED[i].DOC_STATUS != "2"){
...
function fnApprSave() {
	...
	saveCall(gridView, '/com/wrh/apprReceived', "fnAppr", params);
}
...
var fnReturn = function(){
	...
	for(var i = 0; i < params.ITEM_LIST.UPDATED.length; i++){
		if(params.ITEM_LIST.UPDATED[i].IMP_MAT_YN == "Y"){
...
function fnReturnSave() {
	...
	saveCall(gridView, '/com/wrh/returnReceivedAppr', "fnReturn", params);
}
