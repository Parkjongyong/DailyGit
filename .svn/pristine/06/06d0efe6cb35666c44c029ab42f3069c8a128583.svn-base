...
function setInitGridView() {
    ...
    addField(cm,    'GR_DATE_TIME',                 {text: '입고예정일시'},                     100,     'textLink', {textAlignment: "center"});
    ...
    addField(cm,    'COMP_NM',                      {text: '회사코드'},                          60,     'text', {textAlignment: "center"});
    ...
    gridView.onDataCellClicked = function (grid, colIndex) {
        if (colIndex.fieldName === "DOC_NO") {
        	fnSearchDetail();
        } else if (colIndex.fieldName === "GR_DATE_TIME"){
    		if(grid.getCurrentRow().GR_DELY_DATE <= $("#TB_SYSDATE").val().replaceAll("-","")){
    			alert("입고예정일시가 미래인 데이터만 수정 가능합니다.");
    			return false;
    		}
        	fnSearchDelyDate(colIndex);
        } else if (colIndex.fieldName === "CODEMAPPING1") {
...
function fnSearch() {
	// 조회 요청
	searchCall(gridView, '/com/wrh/selectScheduledMgmt');
}
...
function loadNext() {
    ...
    ajaxJsonCall('<c:url value="/com/wrh/selectScheduledMgmt.do"/>',  params, function(data){
...
function fnSearchSuccess(data) {
	if(isEmpty(data.rows)){
		...
		$("#TB_H_DOC_NO").val(data.rows[0].DOC_NO);
	}
	// 에러메세지 처리
	alertErrMsg(data);
	// 그리드 초기화
...
function fnSave() {
	...
	for(var i = 0; i < params.ITEM_LIST.UPDATED.length; i++){
		if(params.ITEM_LIST.UPDATED[i].GR_DELY_DATE.replaceAll("-","") <= $("#TB_SYSDATE").val().replaceAll("-","")){
	...
	if(fnSaveCheck(gridView, requiredVal) == true){
	     if(confirm("저장 하시겠습니까?")){
	    	 saveCall(gridView, '/com/wrh/saveScheduledMgmt', null, params);
	     }
	}
}
...
function fnSaveSuccess(result) {
	...
	$.extend(params, fnGetParams());
	$.extend(params, {"ITEM_LIST" : fnGetSaveData(gridView)});
	$.extend(params, {"STATUS_NM" : '일정조정' });
	$.extend(params, {"STAT" : 'AC' });
...
var fnReturn = function(){
	...
	for(var i = 0; i < params.ITEM_LIST.UPDATED.length; i++){
		if(params.ITEM_LIST.UPDATED[i].GR_DELY_DATE < $("#TB_SYSDATE").val().replaceAll("-","")){
			alert("입고예정일시가 미래인 데이터만 가능합니다.");
			return false;
		}
	}
...
function fnCallbackDelyDateSearchPop(rows) {
	var values = {
	       ...
	       , "GR_DATE_TIME"    : rows.DELY_DATE_TIME.substr(0,4) +"-"+rows.DELY_DATE_TIME.substr(4,2)+ "-" + rows.DELY_DATE_TIME.substr(6,2) + rows.DELY_DATE_TIME.substr(8)
	     };
...
<input type="hidden" id="MOBILE_NO"     name="MOBILE_NO" value="<c:out value="${LOGIN_INFO.CELL_PHONE_NO}" />" />
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="hidden" id="TB_GR_DELY_DATE" name="TB_GR_DELY_DATE" value="">
                        <input type="hidden" id="TB_H_DOC_NO" name="TB_H_DOC_NO" value="">
                    </td>
