<%--
	File Name : cmnPlantStoragePop.jsp
	Description: 공통 플랜트 창고 검색 팝업
...
<script type="text/javascript">
	...
	function setInitGrid() {
	    ...
	    addField(cm ,   'COMP_NM'     , {text: '회사'}    ,0       , 'text'            , {textAlignment: "center"},{visible:false});
	    addField(cm ,   'COMP_CD'     , {text: '회사코드'} ,0       , 'text'            , {textAlignment: "center"} ,{visible:false});
	    addField(cm ,   'PLANT_CD'    , {text: '플랜트'}  ,80      , 'text'            , {textAlignment: "center"});
	    addField(cm ,   'PLANT_NM'    , {text: '플랜트명'} ,120     , 'text'            , {textAlignment: "center"});
	    addField(cm ,   'STORAGE_CD'  , {text: '창고'}    , 80     , 'text'            , {textAlignment: "center"});
	    addField(cm ,   'STORAGE_NM'  , {text: '창고명'}   , 120    , 'text'            , {textAlignment: "center"});
	...
	function fnSearch() {
		// 조회 요청
		searchCall(gridView, '/com/cmn/selectPlantStorageList');
	}

	//부모창에 선택값 셋팅함수 호출
	function setOpenerData(rowData) {
		parentCallback('fnCallbackPlantStoagePopup',rowData);
	}
		        ...
		        <table class="tbl-search">
					...
					<tbody>
						<tr>
							<th><span>회사</span></th>
							<td>
							     <input type="text" id="TB_COMP_NM" name="TB_COMP_NM" class="w150 enterEvnt" value="${PARAM.SB_COMP_NM}" readOnly/>
							     <input type="hidden" id="TB_COMP_CD" name="TB_COMP_CD" value="${PARAM.SB_COMP_CD}"/>
							</td>
							<td></td>
						</tr>
					</tbody>
