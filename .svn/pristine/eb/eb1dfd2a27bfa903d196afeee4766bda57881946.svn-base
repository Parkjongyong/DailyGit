...
<script type="text/javascript">
	...
	function setInitGrid() {
	    ...
	    addField(cm,    'VENDOR_CD',        {text: '업체코드'},    70,     'text', {textAlignment: "center"} ,  {editable:false});
	    addField(cm,    'VENDOR_NM',      	{text: '업체명'},     120,     'text', {textAlignment: "near"}   ,  {editable:false});
	    addField(cm,    'POBUSI_NO',   		{text: '사업자번호'},   70,     'text',  {textAlignment: "center"},  {editable:false});
	...
	function fnSearch() {
		// 조회 요청
		searchCall(gridView, '/com/cmn/selectVendorList');
	}

	//부모창에 선택값 셋팅함수 호출
	function setOpenerData(rowData) {
		parentCallback('fnCallbackVendorSearchPop',rowData);
	}
