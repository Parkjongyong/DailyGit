...
@Controller
public class makeDepreAccountInterfaceController extends BaseJobBean {
	...
	private String _BATCH_JOB_ID	= "MAKE_DEPRE_ACCOUNT_IF";
	...
	public void makeDepreAccountInterfaceJobRun(LinkedHashMap<String, Object> serviceParams) throws Exception {
		String compCd		= "";
		String runIp		= "";
			...
			try {
	        	...
	        	for(int i = 0; i < compInfo.size(); i++) {
		            ...
		            if (!"".equals(StringUtil.isNullToString(paramInfo.get("BATCH_ARG6")))) {
		            	...
		            	System.out.println("회사연도::" + paramInfo.get("BATCH_ARG6").toString());
		            } else {
		            	System.out.println("회사연도(디폴트처리)::" + uDate.format(cal.getTime()).substring(1, 4));
		            	dt.setI_GJAHR(uDate.format(cal.getTime()));
		            }
		            
		            if (!"".equals(StringUtil.isNullToString(paramInfo.get("BATCH_ARG7")))) {
		            	dt.setI_VERSN(paramInfo.get("BATCH_ARG7").toString());
		            	paramMap.put("VERSION", paramInfo.get("BATCH_ARG7").toString());
		            	System.out.println("버전::" + paramInfo.get("BATCH_ARG7").toString());
		            	System.out.println("버전2::" + paramInfo.get("BATCH_ARG7").toString().subSequence(0, 1));
		            } else {
		            	System.out.println("버전(디폴트처리)::A00");
