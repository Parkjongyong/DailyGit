<%--
	File Name : sysUserRolePopup.jsp
...
function fnLeftSearch() {
    ...
    $.extend(params, {S_USER_ID   : '${PARAM.USER_ID}'});
    $.extend(params, {MODULE_TYPE : '${PARAM.USER_CLS}'});
	// 조회 요청
	searchCall(gridList1, '/com/sys/selectUnassignedRoleList', 'fnLeftSearch', params);    
}
...
function fnRightSearch() {
    ...
    var params = fnGetParams();
    $.extend(params, {S_USER_ID : '${PARAM.USER_ID}'});
    $.extend(params, {MODULE_TYPE : 'B'});
	// 조회 요청
	searchCall(gridList2, '/com/sys/selectAssignedRoleList', 'fnRightSearch', params); 
}
...
function fnSave() {
	...
	if(confirm("저장 하시겠습니까?")){
	    ...
	    saveCall(gridList2, '/com/sys/updateUserRole', null, params);
    }	
}
